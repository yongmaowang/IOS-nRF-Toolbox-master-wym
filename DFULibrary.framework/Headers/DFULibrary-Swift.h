...
@property (nonatomic, readonly, copy) NSString * __null_unspecified fileName;

/// The URL to the firmware file.
@property (nonatomic, readonly, strong) NSURL * __null_unspecified fileUrl;

/// Information whether the firmware was successfully initialized.
@property (nonatomic, readonly) BOOL valid;

/// The size of each component of the firmware.
@property (nonatomic, readonly, strong) DFUFirmwareSize * __nonnull size;
...
- (nullable instancetype)initWithUrlToZipFile:(NSURL * __nonnull)urlToZipFile OBJC_DESIGNATED_INITIALIZER;
...
///
/// \returns  the DFU firmware object or null in case of an error
- (nullable instancetype)initWithUrlToBinOrHexFile:(NSURL * __nonnull)urlToBinOrHexFile urlToDatFile:(NSURL * __nullable)urlToDatFile type:(enum DFUFirmwareType)type OBJC_DESIGNATED_INITIALIZER;
@end
...
- (BOOL)select:(CBPeripheral * __nonnull)peripheral advertisementData:(NSDictionary<NSString *, id> * __nonnull)advertisementData RSSI:(NSNumber * __nonnull)RSSI;
...
- (NSArray<CBUUID *> * __nullable)filterBy;
@end
...
- (void)didErrorOccur:(enum DFUError)error withMessage:(NSString * __nonnull)message;
@end
...
@property (nonatomic, strong) id <DFUServiceDelegate> __nullable delegate;

/// An optional progress delegate will be called only during upload. It notifies about current upload percentage and speed.
@property (nonatomic, strong) id <DFUProgressDelegate> __nullable progressDelegate;

/// The logger is an object that should print given messages to the user. It is optional.
@property (nonatomic, strong) id <LoggerDelegate> __nullable logger;
...
@property (nonatomic, strong) id <DFUPeripheralSelector> __nonnull peripheralSelector;
...
- (nonnull instancetype)initWithCentralManager:(CBCentralManager * __nonnull)centralManager target:(CBPeripheral * __nonnull)target OBJC_DESIGNATED_INITIALIZER;
...
- (DFUServiceInitiator * __nonnull)withFirmwareFile:(DFUFirmware * __nonnull)file;
...
- (DFUServiceController * __nullable)start;
@end
...
- (void)logWith:(enum LogLevel)level message:(NSString * __nonnull)message;
@end
